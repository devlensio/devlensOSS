    ...
    <main className="min-h-screen bg-base text-primary">
      <div className="px-8 py-8">
        ...
        <div className="bg-surface border border-border rounded-xl p-6 mb-8">
          ...
          {analyze.isError && (
            <div className="mt-4 text-sm bg-error/10 border border-error/20 rounded-lg px-4 py-2.5">
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-primary">Analyzed Repositories</h2>
        ...
        {isLoading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-44 rounded-xl bg-surface border border-border animate-pulse"
              />
            ))}
          </div>
        )}
        ...
        {!isLoading && (!graphs || graphs.length === 0) && (
          <div className="text-center py-20 border border-dashed border-border rounded-xl">
            <div
              className="w-12 h-12 rounded-xl bg-elevated border border-border 
            ...
            <p className="text-muted text-sm">
              Enter a path above to analyze your first repo.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-start">
          {graphs?.map((graph: GraphListItem) => (
            <RepoCard key={graph.graphId} graph={graph} />
          ))}
        </div>
      </div>
    </main>
