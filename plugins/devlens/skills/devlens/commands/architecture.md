...
- **Write each section completely before querying for the next.** Do not collect all data upfront then write everything at the end — by then the context is full of JSON and the write suffers. Instead: finish §1 and §2 from steps 1–2 data, then query for §3 flows (steps 4–6), write §3, then query for health (step 7), write §8, etc. This prevents the "duplicate section" failure where the model loses its place and restarts a section mid-stream.
- **Protect the synthesis budget.** Batch `get_summaries`, cap route khop to 5–8 routes (step 4), cap get_subgraph to 5 nodes (step 2). If budget runs short, **a few sections written cleanly beats all sections truncated**; a garbled brief is a failure even if the data behind it was right.
